...
function playlist(force_dir)
  ...
  if files then
    ...
    if c2 > 0 or c>0 then
      msg.info("Добавлено "..c + c2.." файлов в плейлист")
    else
      msg.info("Доп. файлов не найдено")
    end
    cursor = mp.get_property_number('playlist-pos', 1)
  else
    msg.error("Не получается просканировать файлы: "..(error or ""))
  end
  refresh_globals()
  if playlist_visible then
    showplaylist()
...
function save_playlist(filename)
  ...
  if utils.readdir(savepath) == nil then
    ...
    if res.status ~= 0 then
      msg.error("Не получилось создать директорию для сохранения плейлиста "..savepath..". Error: "..(res.error or "unknown"))
      return
    end
  end
  ...
  if not file then
    msg.error("Ошибка при создании файла плейлиста, глянь разрешения. Ошибка: "..(err or "unknown"))
  else
    file:write("#EXTM3U\n")
    local i=0
    while i < length do
    ...
    local saved_msg = "Плейлист записан тут: "..savepath
...
function sortplaylist(startover)
  ...
  if settings.display_osd_feedback then
    mp.osd_message("Плейлист отсортирован "..sort_modes[sort_mode].title)
  end
end
...
function reverseplaylist()
  ...
  if playlist_visible then
    showplaylist()
  elseif settings.display_osd_feedback then
    mp.osd_message("Плейлист реверсирован")
  end
end
...
function shuffleplaylist()
  ...
  if playlist_visible then
    showplaylist()
  elseif settings.display_osd_feedback then
    mp.osd_message("Плейлист перемешан")
  end
end
