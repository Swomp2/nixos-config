	...
	if pos then
		for _, i in pairs(ranges) do
			...
			if i.segment[1] <= pos and v > pos then
				...
				mp.osd_message(("[sponsorblock] пропуск %ds"):format(math.floor(v-mp.get_property("time-pos"))))
...
function toggle()
	if ON then
		mp.unobserve_property(skip_ads)
		mp.osd_message("[sponsorblock] выключен")
		ON = false
	else
		mp.observe_property("time-pos", "native", skip_ads)
		mp.osd_message("[sponsorblock] включен")
		ON = true
	end
end
