...
function make_gif_internal(burn_subtitles)
    ...
    if start_time_l == -1 or end_time_l == -1 or start_time_l >= end_time_l then
        mp.osd_message("Неверное начало/конец.")
        return
    end

    mp.osd_message("Создаю GIF.")
    ...
    if not gifname then
        mp.osd_message('Нет свободных имён файлов:(')
        return
    end
	...
	if ok then
	    ...
	    mp.osd_message("GIFка создана: " .. gifname)
	else
	    mp.osd_message("Ошибка создания файла, смотри вывод в терминале.")
	end
end

function set_gif_start()
    start_time = mp.get_property_number("time-pos", -1)
    mp.osd_message("Начало GIFки : " .. start_time)
end

function set_gif_end()
    end_time = mp.get_property_number("time-pos", -1)
    mp.osd_message("Конец GIFки : " .. end_time)
end
